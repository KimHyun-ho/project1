...
public class ScheduleController {
	...
	@ResponseBody
	public List<Map<String, Object>> selectScheduleDoctorArrJson(HttpSession session) throws Exception {
		...
		for (int i = 0; i < scheduleDoctorList.size(); i++) {
			...
			scheduleDoctorHash.put("scheduleDoctorWriteDate", scheduleDoctorList.get(i).get("scheduleDoctorWriteDate")); // 스케줄
																															// 작성
																															// 일자
			scheduleDoctorHash.put("scheduleDoctorUpdateDate",
					scheduleDoctorList.get(i).get("scheduleDoctorUpdateDate")); // 스케줄 수정 일자

			// 중괄호 {key:value , key:value, key:value}
			jsonObj = new JSONObject(scheduleDoctorHash);
			// 대괄호 안에 넣어주기[{key:value , key:value, key:value},{key:value , key:value,
			// key:value}]
			jsonArr.add(jsonObj);
	...
	@ResponseBody
	public String insertScheduleDoctor(@RequestBody List<Map<String, Object>> scheduleDoctorList, HttpSession session)
			throws Exception {
	...
	@ResponseBody
	public String updateScheduleDoctor(@RequestBody List<Map<String, Object>> scheduleDoctorList, HttpSession session)
			throws Exception {
		// 화면에서 추가한 캘린더에 추가한 데이터가 잘 넘어왔는지 찍어보기
	...
	@ResponseBody
	public String deleteDoctorSchedule(@RequestBody List<Map<String, Object>> scheduleDoctorList, HttpSession session) throws Exception {
		// 화면에서 추가한 캘린더에 추가한 데이터가 잘 넘어왔는지 찍어보기
	...
	@ResponseBody
	public List<Map<String, Object>> proDoctorSchedule(@RequestParam int seqDoctor ,HttpSession session) throws Exception {
		...
		for (int j = 0; j < scheduleProtectorList.size(); j++) {
			...
			scheduleProtectorHash.put("scheduleProtectorWriteDate",
					scheduleProtectorList.get(j).get("scheduleProtectorWriteDate")); // 스케줄 작성 일자
			scheduleProtectorHash.put("scheduleProtectorUpdateDate",
					scheduleProtectorList.get(j).get("scheduleProtectorUpdateDate")); // 스케줄 수정 일자
	...
	@ResponseBody
	public List<Map<String, Object>> proScheduleOk(HttpSession session) throws Exception {
		...
		for (int i = 0; i < scheduleProtectorList.size(); i++) {
			...
			scheduleProtectorHash.put("scheduleProtectorWriteDate",
					scheduleProtectorList.get(i).get("scheduleProtectorWriteDate")); // 스케줄 작성 일자
			scheduleProtectorHash.put("scheduleProtectorUpdateDate",
					scheduleProtectorList.get(i).get("scheduleProtectorUpdateDate")); // 스케줄 수정 일자
			scheduleProtectorHash.put("color", "#008000"); // 색깔 지정
	...
	@ResponseBody
	public String insertScheduleProtector(@RequestBody List<Map<String, Object>> scheduleProtectorList, HttpSession session)
			throws Exception {
		// 화면에서 추가한 캘린더에 추가한 데이터가 잘 넘어왔는지 찍어보기
		...
		for (Map<String, Object> list : scheduleProtectorList) {
			...
			startDate = LocalDateTime.parse(startDateString, dateTimeFormatter).plusHours(9);
			endDate = LocalDateTime.parse(endDateString, dateTimeFormatter).plusHours(9);
			startDateToString = "%" + startDate.toString().substring(0, 10) + " "
					+ startDate.toString().subSequence(11, 16) + "%";
		}
		int seqDoctor = (int) session.getAttribute("seqDoctor");
		...
		insertScheduleProtectorMap.put("scheduleProtectorHolidayStart",
				scheduleDoctororMap.get("scheduleDoctorHolidayStart"));
		insertScheduleProtectorMap.put("scheduleProtectorHolidayEnd",
				scheduleDoctororMap.get("scheduleDoctorHolidayEnd"));
	...
	@ResponseBody
	public String updateScheduleProtector(@RequestBody List<Map<String, Object>> scheduleProtectorList, HttpSession session)
			throws Exception {
		// 화면에서 추가한 캘린더에 추가한 데이터가 잘 넘어왔는지 찍어보기
		...
		for (Map<String, Object> list : scheduleProtectorList) {
			...
			endDate = LocalDateTime.parse(endDateString, dateTimeFormatter).plusHours(9);
			startDateToString = "%"
					+ LocalDateTime.parse(startDateString, dateTimeFormatter).toString().substring(0, 10) + " "
					+ LocalDateTime.parse(startDateString, dateTimeFormatter).toString().subSequence(11, 16) + "%";
		}
		// 의사 스케줄 seq값을 조회할 조건 시간 맵에 담기
		Map<String, String> startEndMap = new HashMap<>();
		startEndMap.put("startDate", startDateToString);
	...
	@ResponseBody
	public String deleteScheduleProtector(@RequestBody List<Map<String, Object>> scheduleProtectorList)
			throws Exception {
		// 화면에서 추가한 캘린더에 추가한 데이터가 잘 넘어왔는지 찍어보기
	...
	public String selectScheduleDoctor(Model model, HttpSession session) throws Exception {
		// session에서 받아야 할 값
		int seqDoctor = (int) session.getAttribute("seqDoctor");
		// 데이터 담을 그릇 준비
	...
	public String insertTodolist(@RequestParam Map<String, Object> todolistMap, HttpSession session) throws Exception {
