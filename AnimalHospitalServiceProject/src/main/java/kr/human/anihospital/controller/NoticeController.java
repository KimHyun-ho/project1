...
public class NoticeController {
	...
	public String selectAllNoticeList(@RequestParam(defaultValue = "1") int c, @RequestParam(defaultValue = "10") int p,
			@RequestParam(defaultValue = "10") int b, Model model) throws Exception {
		// notice화면에 표시할 값 가져오기
		// 시작-------------------------------------------------------------

		// 모든 공지를 담을 그릇 준비
		// 준비한 그릇에 화면에 표시할 공지 내용을 담는다.
		PagingVO<AllNoticeListVO> pagingVO = noticeService.selectAllNoticeListVO(c, p, b);
		// 받아온 데이터 화면에 넘겨주기
		model.addAttribute("allNoticeList", pagingVO.getList());
		model.addAttribute("info", pagingVO.getInfo());
		model.addAttribute("pageList", pagingVO.getPageList());
		// 제대로 데이터가 담겨 있는지 로그에 찍어보기
		log.info("selectAllNoticeList 메서드 호출(컨트롤러) : {}", pagingVO.getList());
	...
	public String selectInsertNoticeWriterInfo(Model model) throws Exception {
		// insert화면에 로그인 한 의사 정보 표시할 select
		// 시작-----------------------------------------------
